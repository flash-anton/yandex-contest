 ...
 * <a href="https://contest.yandex.ru/contest/27393/run-report/92514524">OK  151ms  11.37Mb</a>
 *
 * H. Метро
 ...
 * из другого часового пояса, то из-за джетлага сильно проспала, и Тане пришлось долго её ждать. Поезда всегда ходят
 * точно по расписанию, и Таня знает, что поезд стоит на платформе ровно одну минуту, а интервал между поездами
 * (время, в течение которого поезда у платформы нет) составляет a минут для поездов на первом пути и b минут для
 * поездов на втором пути. То есть на первый путь приезжает поезд и стоит одну минуту, затем в течение a минут поезда
 * у платформы нет, затем в течение одной минуты у платформы стоит следующий поезд и т. д.
 *
 * Пока Таня стояла на платформе, она насчитала n поездов на первом пути и m поездов на втором пути. Определите
 * минимальное и максимальное время, которое Таня могла провести на платформе, или сообщите, что она точно сбилась со счёта.
 *
...
public class H {
    ...
    public static void alg(BufferedReader reader, BufferedWriter writer) throws IOException {
        ...
        int aMinTime = 1 + (a + 1) * (n - 1);
        int aMaxTime = a + aMinTime + a;
        int bMinTime = 1 + (b + 1) * (m - 1);
        int bMaxTime = b + bMinTime + b;

        int commonMinTime = Math.max(aMinTime, bMinTime);
        int commonMaxTime = Math.min(aMaxTime, bMaxTime);

        String solution;
        if (commonMinTime > commonMaxTime) {
            solution = "-1";
        } else {
            solution = commonMinTime + " " + commonMaxTime;
        }
