...
@DisplayName("PrefixHashesSimpleTest")
class PrefixHashesSimpleTest extends ContestChecker {
    ...
    public static String alg2(String S, int Q, String[] requests) {
        StringBuilder sb = new StringBuilder();
        var ph = new PrefixHashesSimple(27, 1_000_000_007, 200_000);
        var h = ph.hashes(S.getBytes());
        for (int i = 0; i < Q; i++) { // O (Q + N)
            int[] ints = Arrays.stream(requests[i].split(" ")).mapToInt(Integer::valueOf).toArray();
            int L = ints[0];
            int A = ints[1];
            int B = ints[2];
            String s = ph.isEquals(h, A, h, B, L) ? "yes" : "no"; // first: O(L)->O(N), other: O(1)
            sb.append(s).append("\n");
        }
        return sb.toString();
    }
    ...
    public static String alg3(String S, String S2, int Q, String[] requests) {
        StringBuilder sb = new StringBuilder();
        var ph = new PrefixHashesSimple(27, 1_000_000_007, 200_000);
        var h1 = ph.hashes(S.getBytes());
        var h2 = ph.hashes(S2.getBytes());
        for (int i = 0; i < Q; i++) { // O (Q + N)
            int[] ints = Arrays.stream(requests[i].split(" ")).mapToInt(Integer::valueOf).toArray();
            int L = ints[0];
            int A = ints[1];
            int B = ints[2];
            String s = ph.isEquals(h1, A, h2, B, L) ? "yes" : "no"; // first: O(L)->O(N), other: O(1)
            sb.append(s).append("\n");
        }
        return sb.toString();
    }
